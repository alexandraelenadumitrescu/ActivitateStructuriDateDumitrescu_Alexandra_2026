...
HashTable initializareHashTable(int dimensiune) {
	...
	//trebuie sa setam fiecare ptr din vector cu null
...
int calculeazaHash(int id, int dimensiune) {//de facut clusterizare dupa serie/model
...
void inserareMasinaInTabela(HashTable hash, Masina masina) {
	...
	if (!hash.vector[hashCode]) {
		//daca nu exista-> nu avem coliziune
...
HashTable citireMasiniDinFisier(const char* numeFisier) {
	...
	HashTable ht = initializareHashTable(6);
	if (f) {
		while (!feof(f)) {
			Masina citita = citireMasinaDinFisier(f);
			inserareMasinaInTabela(ht, citita);
		}
		fclose(f);
	}
	return ht;
...
void afisareTabelaDeMasini(HashTable ht) {
	...
	for (int i = 0;i < ht.dim;i++) {
		printf("cluster ul %d \n", i + 1);
		afisareListaDeMasini(ht.vector[i]);
		printf("===================\n");
	}
...
Masina getMasinaDupaId(HashTable ht, int id) {
	...
	int hashCode = calculeazaHas(id, ht.dim);
